...
export function parseClassifiedsPage(html, options = {}) {
  ...
  for (const el of $('div.node.node-classified.node-teaser').get()) {
    ...
    // (id="title975570"). Scoped to this listing's own heading.
    const $titleHeading = $block.find('h2.title').first();
    const titleId = $titleHeading.attr('id') ?? '';
    const nodeIdMatch = /(\d+)$/.exec(titleId);
    if (!nodeIdMatch) throw new Error('classifieds: listing without a numeric node id');
    ...
    if (absolute) {
      ...
      if (!relative) throw new Error(`classifieds: no recognised timestamp in listing ${nodeId}`);
