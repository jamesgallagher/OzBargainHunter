 ...
 *   1. The rule's interest is established first: a match rule's term must
 *      match, and a threshold rule's deal must be at or near the threshold.
 *      Only a record the rule would have acted on may be recorded.
 *   2. Expiry. An expired deal never alerts, in any circumstance, including
 *      one that became expired after it was first seen. Expiry is evaluated
 *      at emit time and recorded as `expired` only for a record the rule is
 *      interested in — ahead of the near-miss classification.
 *   3. Ledger. Keyed on (node ID, rule ID); a rule fires at most once per
 *      deal, ever. A permanent record, not a cooldown.
 *   4. Per-rule cooldown, default 24 hours, configurable per rule, zero
 *      permitted.
 *   5. Repost suppression. A new record whose normalised title is close to
 ...
 *      sets, threshold 0.60.
 *   6. Muted rules keep evaluating and keep writing ledger entries; they
 *      send nothing.
 *
...
function evaluateRecordRule(record, surface, rule, ctx) {
  ...
  // 1. The rule's interest is established first: a match rule's term must
  // match, and a threshold rule's deal must be at or near the threshold.
  // Only a record the rule would have acted on may be recorded — a rule
  // matching nothing records nothing.
  ...
  if (rule.type === 'match') {
    ...
    thresholdReached = reachedThreshold(rule, record, nowIso);
    const near = !thresholdReached && net >= rule.threshold * 0.9 && withinWindow;
    // A record the rule is not interested in (neither reached nor a near
    // miss) records nothing. A deal that has aged out of a window is not a
    // near miss — the rule simply cannot fire on it.
    if (!thresholdReached && !near) return { action: 'no_reach' };
  }

  // 2. Expiry (deals/front only; evaluated at emit time). An expired deal
  // never alerts, in any circumstance — including one that became expired
  // after it was first seen. This runs ahead of the near-miss
  // classification, so an expired deal the rule is interested in is recorded
  // as `expired`, not as a `near_miss`.
  ...
  // 3. A threshold deal the rule is interested in but that has not reached
  // the threshold is a near miss (within 10% below, inside the window).
  if (rule.type === 'threshold' && !thresholdReached) {
  ...
  // X11: a snoozed rule lapses when its `snooze_until_<id>` instant passes —
  // the engine re-enables it (state='enabled', clears the instant) and the
  // rule resumes as if it were never snoozed. This is the only place a
  // snoozed rule is re-enabled; the UI's undo/re-enable is the other. A
  // snoozed rule whose instant has not yet passed suppresses, like a muted
  // rule (it keeps evaluating and writing a ledger entry, sends nothing).
  if (rule.state === 'snoozed') {
    const until = ctx.store.getSetting(`snooze_until_${rule.id}`);
    if (until && Date.parse(until) <= Date.parse(nowIso)) {
      // Lapse: re-enable and clear the instant.
      ctx.store.setRuleState(rule.id, 'enabled', nowIso);
      ctx.store.deleteSetting(`snooze_until_${rule.id}`);
    } else {
      ctx.store.insertLedger({
        node_id: record.node_id,
        rule_id: rule.id,
      ...
      // The suppression kind is 'muted' (a snoozed rule is off); 'snoozed' is
      // the *state*, not a suppression kind (the suppressions table CHECK does
      // not admit it).
