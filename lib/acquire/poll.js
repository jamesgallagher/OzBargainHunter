 ...
 *
 * The access gate (design 3.7) governs the whole cycle: a closed gate
 * (cooling / stopped) makes the cycle a no-op — zero requests, zero
 * failure rows, no poll_state write, no deals-cycle record (no catch-up);
 * a probing gate fetches exactly the probe URL (deals page 0), once, then
 * ends the cycle whatever the result. After every URL the gate is re-read;
 * a gate that closed mid-cycle stops the remaining requests and the cycle
 * commits what it already fetched.
 *
 * Per URL:
 ...
 *   BlockedError with `gateState` — the access gate (design 3.7) refused
 *          the request: no request went out, so it is NOT a failure (no
 *          failures row); log it and stop issuing further requests.
 *   BlockedError without `gateState` — a client latched off by a Cloudflare
 *          block (the legacy latch, kept for existing tests): record a
 *          failures row (class cloudflare_block), stop issuing further
 *          requests, and commit what the cycle already fetched.
 ...
 * consecutive_failures counter) — except when the gate is closed, in which
 * case the cycle is a no-op and poll_state is left untouched.
 */
...
/**
 * @param {{
 *   client: { request(url: string, options?: object): Promise<{ class: string, status: number, body: string }> },
 *   store: object,
 *   clock: { now(): Date },
 *   config: object,
 ...
 *   gate?: object,
 * }} deps
 * @returns {Promise<{ frontPageAvailable: boolean, failures: number, lastResponseClass: string, skipped?: 'gate_closed' }>}
 */
export async function runDealPoll(deps) {
  const { client, store, clock, config, log = console.log } = deps;
  // The access gate (design 3.7). The worker builds one gate and shares it
  // across the deal and classifieds polls; a caller that does not supply
  // one (the existing acquisition tests) gets a gate over the same store,
  // so both gates read and write the same row.
  ...
  // The gate governs the whole cycle (design 3.7): a closed gate (cooling
  // / stopped) makes the cycle a no-op — zero requests, zero failure rows,
  // no poll_state write, no deals-cycle record (no catch-up). The early
  // return bypasses the finally below on purpose.
  const mode = gate.mode();
  if (mode === 'closed') {
    log(`${clock.now().toISOString()} gate ${gate.read().state}: deal poll skipped`);
    return {
      skipped: 'gate_closed',
      frontPageAvailable: false,
      failures: 0,
      lastResponseClass: null,
      pollAt: clock.now().toISOString(),
      feeds: [],
    };
  }
  ...
  // The feeds the caller hands to the rules engine. A 304 page contributes
  // nothing — it did not change — so only 200 pages add a feed entry. The
  // surface is 'deals' for the two deals pages and 'front' for the front
  // page, matching the engine's front-page priority and the classifieds
  // surface name.
  const feeds = [];
  ...
  let failures = 0;
  // The transient/transport failure count for the gate's deals-cycle
  // signal (design 3.7: a failing cycle is one that reached no deals feed
  // with at least one transient/transport failure). Only those two classes
  // count — a 429/503 or a Cloudflare block closes the gate on its own.
  ...
  // One instant for the whole cycle (design 4.1: "observations — one row per
  // deal per poll"). A poll is one instant; the client's pause between the
  // three requests is pacing, not data. Stamping every observation with the
  // same `observed_at` is what makes the store's (deal_id, observed_at)
  // unique index express "one row per deal per poll" *across writers*: the
  // rules engine upserts the same records it evaluates, and with a shared
  // instant that write collides with this one instead of appending a second
  // row (which would also overwrite this cycle's first-feed-wins counters
  // with the later feed's — `fixtures/README.md` §2 pins 975666 at 113 for
  // poll 1, the page-0 value). The instant is taken before the first request
  // and returned as `pollAt`, so the caller evaluates the feeds at exactly
  // the instant they were observed.
  const pollInstant = nowIso();

  async function handle(url, isFrontPage) {
    let response;
    try {
      response = await client.request(url, { surface: 'deals' });
    } catch (err) {
      ...
      // A BlockedError carrying `gateState` is the access gate (design 3.7)
      // refusing the request: no request went out, so it is NOT a failure —
      // no failures row, no counter. Log it; the loop breaks on
      // !gate.isOpen() and the observations and poll_state are written
      // below.
      if (err?.name === 'BlockedError' && err.gateState !== undefined) {
        log(`${nowIso()} ${url} gate ${err.gateState}`);
        return;
      }
      // A BlockedError without `gateState` is the client already latched
      // off by a Cloudflare block (the legacy latch, kept for existing
      // tests). Record it as cloudflare_block (not transport_error) so the
      // block class is not mislabelled, and feed it to the gate (rule B1
      // closes it) so the loop breaks on !gate.isOpen() below.
      ...
      failures += 1;
      if (responseClass === 'transport_error') transientFailures += 1;
      lastResponseClass = responseClass;
      log(`${nowIso()} ${url} ${responseClass} ${err?.message ?? err}`);
      if (isFrontPage) frontPageAvailable = false;
      // A Cloudflare block closes the gate (design 3.7, rule B1), so the
      // loop breaks on !gate.isOpen() and no further requests go out. The
      // legacy latch carries no flag the loop could test — the gate is the
      // only thing that stops the remaining URLs.
      if (isBlock) gate.recordResponse({ class: 'cloudflare_block', surface: 'deals' });
      return;
    }
    ...
    if (response.class === 'ok') {
      ...
      try {
        ...
        failures += 1;
        // The client has already recorded the 200 as `ok`; unparseable is
        // not a gate signal while open, but it ends a probe (design 3.7).
        gate.recordResponse({ class: 'unparseable', surface: 'deals' });
      ...
      // Hand this page's records to the caller for the rules engine. Only a
      // 200 page gets here, so the feed always carries the records that were
      // just upserted above.
      feeds.push({ surface: isFrontPage ? 'front' : 'deals', records });
      ...
      for (const record of records) {
        store.upsertDeal({
          ...
          front_page_first_seen: isFrontPage ? pollInstant : null,
        });
    ...
    failures += 1;
    if (response.class === 'transient') transientFailures += 1;
  ...
  let loopError = null;
  try {
    // Probing fetches exactly the probe URL (deals page 0), once, then ends
    // the cycle whatever the result (design 3.7: no catch-up).
    const toFetch = mode === 'probe' ? [urls[0]] : urls;
    for (const url of toFetch) {
      await handle(url, url === frontPageUrl);
      if (!gate.isOpen()) {
        // The gate closed (or the probe was consumed): stop all OzBargain
        // requests. Commit what we have.
        break;
      }
    }
  } catch (err) {
    // Capture the loop error (e.g. a DeniedPathError) so the finally below can
    // rethrow it if a flush write fails while unwinding — a flush error must
    // not replace the configuration error (see the flush guard below).
    loopError = err;
    throw err;
  } finally {
    ...
    //
    // Guard: if a flush write throws while the loop is unwinding an error,
    // the flush error would otherwise *replace* the loop error (a finally
    // error masks the in-flight one). Re-throw the loop error instead so the
    // deliberate "a deny-listed configured URL fails loudly as a *configuration*
    // error" contract is preserved — the caller must not see a store error
    // (e.g. SQLITE_BUSY) in place of the DeniedPathError.
    try {
    ...
    // Feed the cycle outcome into the gate (design 3.7): a cycle that
    // reached no deals feed with at least one transient/transport failure
    // is a failing cycle (B5 counts them); a cycle that reached a deals
    // feed resets the counters. A no-op signal writes nothing.
    gate.recordDealsCycle({ reachedDealsFeed: dealsFeedReached, transientFailures });
    } catch (flushErr) {
      // A flush write threw while unwinding the loop error. Re-throw the loop
      // error (the configuration error) so the flush error does not mask it.
      // The flush error is NOT preserved: there is no object left to attach it
      // to (the loop error is rethrown and the flush error is discarded), so it
      // is dropped entirely — only the loop error reaches the caller. (The old
      // `flushErr.cause = flushErr.cause ?? loopError` line was inert: it
      // mutated an object that was immediately discarded, and for a
      // non-extensible Error it could itself throw a TypeError that masked the
      // loop error. It is removed rather than wrapped.) When no loop error is
      // in flight, the flush value is rethrown as-is, whatever it is (Error or
      // not), so a non-Error flush value propagates unmodified.
      if (loopError !== null) {
        throw loopError;
      }
      throw flushErr;
  ...
  return {
    ...
    failures,
    lastResponseClass,
    // The cycle's single observation instant (see `pollInstant` above). The
    // caller evaluates the feeds at this instant, so the engine's own upsert
    // of the same records lands on the same (deal_id, observed_at) key.
    pollAt: pollInstant,
    feeds,
  };
}
