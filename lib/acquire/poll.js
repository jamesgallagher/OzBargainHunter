...
export async function runDealPoll(deps) {
  ...
  async function handle(url, isFrontPage) {
    ...
    try {
      ...
      // recorded as a transient failure and retried forever. Rethrow it.
  ...
  // resets to 0. This is what a front-feed 304 cannot launder: a 304 on the
  // front feed does not zero the counter when the deals feed failed.
  const consecutiveFailures = failures > 0 ? storedFailures + failures : 0;
  ...
  // last_success_at advances only when the cycle made progress: either it
  // recorded no failures at all, or at least one *deals* feed was reached
  // with a 200 or 304. A front-feed 304 alone (deals feed failing) must not
  // certify the poll healthy — otherwise a broken deals feed reads as
  // healthy and the dead-man's switch / /healthz can never fire. When the
  // cycle produced no new success, lastSuccessAt is null and the store's
  // COALESCE preserves the stored value.
  const newSuccess = failures === 0 || dealsFeedReached;
