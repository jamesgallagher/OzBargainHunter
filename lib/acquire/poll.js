...
export async function runDealPoll(deps) {
  ...
  try {
    ...
    store.setPollState({
      ...
      // the flush error is preserved as the cause for diagnosis. The cause
      // assignment is guarded by an `instanceof Error` check: a flush write can
      // throw a non-Error value (a string / number / null), and setting
      // `.cause` on a primitive throws a TypeError that would mask *both* the
      // flush error and the in-flight loop error. A non-Error flush value has
      // no `.cause` to carry, so it is simply rethrown as-is when no loop error
      // is in flight.
      if (loopError) {
        if (flushErr instanceof Error) flushErr.cause = flushErr.cause ?? loopError;
        throw loopError;
      }
      throw flushErr;
    }
