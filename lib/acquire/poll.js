 ...
 *   BlockedError — the access gate (design 3.7) refused the request: no
 *          request went out, so it is NOT a failure (no failures row); log
 *          it and stop issuing further requests. (Every `BlockedError` the
 *          client throws carries `gateState`; the gate is the only thing
 *          that stops the remaining URLs.)
...
export async function runDealPoll(deps) {
  ...
  async function handle(url, isFrontPage) {
    ...
    try {
      ...
      // A transport error (timeout, connection error, TLS) must not abort
      // the cycle (design 3.7 "degrade rather than die"). Record it and
      // continue to the next URL; the observations and poll_state are
    ...
    // than die). A Cloudflare block (403) is already recorded to the gate
    // by the client (rule B1 closes it); the loop then breaks on
    // !gate.isOpen() and the cycle commits what it already fetched.
    store.insertFailure({
      failed_at: nowIso(),
