 ...
 *   unparseable     — a 200 whose body is not a parseable page (empty,
 *                     whitespace-only, truncated, non-HTML, or an unreadable
 *                     listing block). The session is unknown for this
 *                     response: never latches, never alerts, and never reads
 *                     a uid from the bad body (which would read it as 0 and
 *                     expire a session we have no evidence about). The
 *                     cached validators for the URL are dropped so the next
 *                     poll fetches a full 200 instead of a 304.
 *
 ...
 * clears it).
 */
import { parseClassifiedsPage, ClassifiedsPageError } from '../parse/classifieds.js';
...
// The persistent global enable/disable gate for classifieds polling. Stored as
// the string '1'/'0' like the freebie setting; absent (null) means disabled.
// The screen-9 toggle route writes it and the poll reads it on every tick, so
// a change takes effect without a restart.
export const CLASSIFIEDS_ENABLED_KEY = 'classifieds_enabled';
...
/**
 * @param {{
 *   client: { request(url: string, options?: { cookie?: string, surface?: string }): Promise<{ class: string, status: number, body: string }>, blocked?: boolean },
 *   store: object,
 *   clock: { now(): Date },
 *   config: object,
...
export async function runClassifiedsPoll(deps) {
  ...
  // with the deal poll and the client; a caller that does not supply one
  // (the existing acquisition tests) gets a gate over the same store.
  // Classifieds makes zero requests unless the gate is open — it never
  // probes.
  ...
  // The global enable/disable gate (persisted, default disabled). When
  // disabled, make zero classifieds requests regardless of any stored cookie
  // and skip cleanly — no transport-error or session-expired failure row.
  // Read on every tick so a change takes effect without a restart.
  const enabledRaw = store.getSetting(CLASSIFIEDS_ENABLED_KEY);
  const enabled = enabledRaw === null ? false : enabledRaw === '1';
  if (!enabled) {
    log(`${nowIso()} ${url} skipped — classifieds polling is disabled`);
    return { state: 'disabled', uid: 0, listings: [], alert: false, latched: false };
  }
  ...
  // Never make an anonymous classifieds request. An expired session remains
  // dormant until the operator supplies a fresh cookie.
  if (!cookie) {
    log(`${nowIso()} ${url} skipped — no classifieds session cookie configured`);
    return { state: 'not_configured', uid: 0, listings: [], alert: false, latched: false };
  }
  if (store.getSetting(LAST_UID_KEY) === '0') {
    log(`${nowIso()} ${url} skipped — classifieds session is expired; set a fresh cookie to retry`);
    return { state: 'expired', uid: 0, listings: [], alert: false, latched: true };
  }

  // The access gate (design 3.7): classifieds makes zero requests unless
  // the gate is open (it never probes). A closed gate is a no-op — no
  // request, no failures row.
  if (!gate.isOpen()) {
    log(`${nowIso()} ${url} skipped — gate ${gate.read().state}`);
    return { state: 'gate_closed', uid: 0, listings: [], alert: false, latched: false };
  }

  let response;
  try {
    response = await client.request(url, { cookie, surface: 'classifieds' });
  } catch (err) {
    // A BlockedError carrying `gateState` is the access gate (design 3.7)
    // refusing the request — the gate may have closed between the check
    // above and this call. No request went out, so it is NOT a failure:
    // no failures row.
  ...
  // A 200: parse the page to read the authoritative uid. A body that is not
  // a parseable page (empty, whitespace-only, truncated, non-HTML, or an
  // unreadable listing block) is unparseable — the session is unknown for
  // this response. It must never be read as uid 0 (which would expire a
  // session we have no evidence about), never latch or alert, and never
  // touch the persisted last uid / confirmed instant. The cached validators
  // for the URL are dropped so the next poll fetches a full 200 rather than
  // a 304 that would hide the repeated bad body.
  let parsed;
  try {
    parsed = parseClassifiedsPage(response.body, { now: clock.now() });
  } catch (err) {
    if (!(err instanceof ClassifiedsPageError)) throw err;
    // The client may be a test stub without the method; the production
    // client always has it.
    if (client.forgetValidators) client.forgetValidators(url);
    // The client has already recorded the 200 as `ok`; unparseable is not
    // a gate signal while open, and classifieds never probes — this is a
    // no-op kept for symmetry with the deals poll (design 3.7).
    gate.recordResponse({ class: 'unparseable', surface: 'classifieds' });
    store.insertFailure({
      failed_at: nowIso(),
      response_class: 'unparseable',
      body: response.body,
    });
    log(`${nowIso()} ${url} unparseable (${err.reason}) — session unknown, not latching`);
    return { state: 'unknown', uid: 0, listings: [], alert: false, latched: false };
  }
