...
export function createOzbClient(deps) {
   ...
   * the Cloudflare latch, and backoff. Requests are serialized: a request
   * that arrives while another is in flight waits for it to finish, then
   * proceeds — never two in flight, and a concurrent caller is queued rather
   * than rejected (the client is shared by the deal and classifieds polls,
   * which can fire at the same instant).
   * @param {string} url
  ...
  async function request(url, options = {}) {
    // Fast path: an already-latched client throws before queuing.
    if (blocked) {
  ...
  async function executeRequest(url, options = {}) {
    // Re-check the latch: the client may have latched off while this request
    // was queued behind an in-flight request.
    if (blocked) {
      throw new BlockedError();
    }
