 ...
 * config. The SMTP server auth comes from config; the target comes from the row.
 */
...
export const MECHANISMS = [
  {
    kind: 'email',
    label: 'Email (SMTP)',
    addLabel: 'Add Email delivery',
    fields: [
      { name: 'to', label: 'Notification recipient', type: 'email', placeholder: 'you@example.com', required: true },
    ],
    testTemplate: defaultTestTemplate(),
    build: (row, config) => emailProvider(buildEmailTransport(row, config)),
  },
  {
    kind: 'matrix',
    label: 'Matrix',
