...
export function createGate({ store, clock, config = {}, log = () => {} }) {
  ...
  function check(url) {
    ...
    if (gate.state === 'probing') {
      ...
      if (allowed) {
        ...
        // it is granted (design 3.7: "once"), and the grant instant is
        // recorded so an unanswered probe expires after 10 minutes.
        withTransition((row) => ({
          gate: { ...row, probe_used: 1, probe_granted_at: clock.now().toISOString() },
          events: [],
        }));
      }
  ...
  function resume() {
    ...
    withTransition((row) => {
      const { gate: next, events, changed } = apply(row, { kind: 'resume' }, clock.now().getTime(), settings);
