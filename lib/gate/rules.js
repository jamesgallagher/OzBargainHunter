...
/**
 * The gate row as it **effectively** stands at `nowMs` — the lazy
 * transitions (cooling→probing at `until_at`, and the granted probe
 * expiring after the 10-minute liveness window) applied in memory,
 * without writing anything. This is what a read-only surface (e.g.
 * `/healthz`) reports: the state the gate would have after a read, but
 * the row and the event table are left untouched.
 *
 * @param {object} row a gate row (all columns)
 * @param {number} nowMs the current instant
 * @returns {object} a copy of the row with the effective `state`
 */
export function effectiveGate(row, nowMs) {
  const g = { ...row };
  if (g.state === 'cooling' && g.until_at !== null && nowMs >= Date.parse(g.until_at)) {
    g.state = 'probing';
  } else if (
    g.state === 'probing' &&
    g.probe_used === 1 &&
    g.probe_granted_at !== null &&
    nowMs >= Date.parse(g.probe_granted_at) + PROBE_TIMEOUT_MS
  ) {
    g.state = 'cooling';
    g.until_at = null;
  }
...
export function apply(gate, signal, nowMs, settings) {
  ...
  /**
   * Re-cool after a failed probe (transient/transport failure or
   * probe_expired). After a B2 cool-off the consecutive count climbs
   * (the fifth lands on B3); after a B5 cool-off (or a manual resume
   * that seeded rule='B5', b5_tier=0) the B5 tier climbs.
   */
  function reCoolAfterFailedProbe(g, reason, nowMs, s) {
    if (g.rule === 'B2') {
      const consecutive = g.consecutive_b2 + 1;
  ...
  switch (signal.kind) {
    ...
    case 'transport_error': {
      ...
      return reCoolAfterFailedProbe(gate, reasonFor(signal.kind, signal.surface), nowMs, s);
    }
    ...
    case 'probe_expired': {
      ...
      return reCoolAfterFailedProbe(gate, 'probe expired', nowMs, s);
    }
