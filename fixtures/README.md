...
Malformed bodies are built inline in the tests (for example `classifieds-safe-failure.test.js`
serves `not html at all` twice) rather than committed as files, so the corpus stays exactly as
captured.
